...
from experiment.loso.loso_phase_utils import load_split, leakage_check, train_phase1_fold
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional subfolder under outputs/loso/phase1/runs/<tag>/results to avoid overwriting default outputs.",
    )
    ...
    out_dir = (
        ROOT / "outputs" / "loso" / "phase1" / "runs" / args.run_tag / "results"
        if args.run_tag
        else ROOT / "outputs" / "loso" / "phase1" / "results"
    )
